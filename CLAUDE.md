...
- **Phase:** **Phase 3 in progress** — [Day 15](docs/daily/day-15.md) (FastAPI over the gold marts), **steps 1–2 of 8 done** (s1 2026-07-12→13, s2 2026-07-13). Phase 2 complete ✅ (Days 8–14).
- **Day 15 s2 landed** (`d22e8c4`): `api/main.py` rewritten — `get_bq_client()` (`lru_cache` singleton) via `Depends`, `/trending` with ISO `?date=` → `date_key` int at the edge, `Query` bounds (422s pre-warehouse), Pydantic envelope, error handler discriminating on job-error `reason` (`bytesBilledLimitExceeded` verified via 1-byte cap → the class is the library *fallback*). Old f-string `/event-counts` dead. Live-verified vs real BQ; suite **34** green; ruff/black clean.
- **⚠️ Uncommitted at close (commit these first tomorrow):** `tests/test_api_main.py` (7 endpoint tests — L4 handout, 3rd test-file handout in 4 days) + `requirements-dev.txt` (`httpx2`) → `add trending endpoint tests`; docs edits (decisions/day-log/warmups/status) → a `docs:` commit.
- **Next session:** `/start-session` full rehydrate (destroy ran), then Day 15 step 3 (framed) — `/languages/momentum` (`momentum_delta` Optional — `bq show --schema` first) + `/leaderboard`; **Bryan writes the tests**; reconciliation 7,236 / 180,386 / 163,953 (leaderboard = ~327-page loop). Also confirm: `?date=` required (plan said default) — deliberate? Then steps 4–8.
- **Warmups:** item 4 passed ✅ cold 2026-07-12 (retire ≥ 07-19); items 1–3 due ~07-15/17/18. Three L4 test-file handouts (D14, D15 s1, D15 s2) — **next warmup = write a test file cold** (`test_api_main.py` is the target).
- **Known issues:** none blocking. Metadata-DB persistence pattern, 3 members (paused flag · `--reset-dagruns` · stale durations). Observer self-state `"running"` (accepted). Quarantine cleanup manual. `dim_date` static 2024 spine. old `/event-counts` dead ✅ (s2); `transform/event_counts.py` + `run.py` die in step 6. Personal ADC — SA impersonation backlogged. DooD accepted locally. `maximum_bytes_billed` ✅ on every query path. Webhook URL in `.env` (secrets backend = Phase 4).
- **Open decisions:** none — date-param shape resolved (ISO `?date=` typed `date`, converted to the `date_key` int at the edge; why in [docs/decisions.md](docs/decisions.md)).
