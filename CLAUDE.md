...
- **Phase:** **Phase 3, one item left** — [Day 16](docs/daily/day-16.md) **COMPLETE** (2026-08-05, three sessions). Day 15 ✅, Phase 2 ✅. **Next up: [Day 17 — hermetic CI on GitHub Actions](docs/daily/day-17.md)** (planned 2026-08-06, not started): 3 jobs — lint · pytest **93** · `dbt parse` — plus pinned deps and required checks on a real PR. **Scoped hermetic on purpose:** `dbt build`/GE need GCP creds and hard rule #4 forbids an SA key, so the credentialed tier (Workload Identity Federation) is *designed in decisions.md, not built*. **No rehydrate, no `terraform apply`, $0 spend.** Then Phase 4 (streaming stretch) or the 🔐 pre-deploy gate (auth · rate limits · lockfile).
