...
| API (Phase 3, localhost-only) | `api/` — `/trending` · `/languages/momentum` · `/leaderboard` (billed query jobs: every value a bound `@param`, `maximum_bytes_billed=100 MB`, TTL cache 300 s + `X-Cache`) · `/runs` (free `list_rows`, uncached, `errors[]` in-band) · host suite **69** tests |
...
- **Phase:** **Phase 3 in progress** — [Day 16](docs/daily/day-16.md) **session 1 done** (step 1 of 7; 2026-07-21). Day 15 complete ✅, Phase 2 complete ✅. **Next: Day 16 step 2** — trending panel (`st.date_input` → the API's required `?date=`, empty state, the `bq` leg of the three-way check vs `movie-web/movie-web` **111 stars** rank 1). Then steps 3–7, then Day 17: GitHub Actions CI (last Phase-3 item).
- **Day 16 s1 landed** (07-21): `dashboard/api_client.py` — one `_get()` door translating every `requests` failure into `DashboardError` (mirror of the `google.api_core → HTTPException` translation in `api/main.py`); the error **carries `status_code`** so panels can tell a routine `/runs` 404 from a real 502; `_get_detail()` guards the error-path `resp.json()`. Timeouts **measured then split** `(connect, read)` per endpoint — marts **878–1183 ms cold / 11–13 ms warm (~75–92×)**, `/runs` flat **179–216 ms** (its *lack* of a cold/warm split re-proves s5's "uncached") → `DEFAULT_TIMEOUT=(1,6)`, `RUNS_TIMEOUT=(1,3)`. Isolated **`.venv-dashboard`** over a container (a container would force `uvicorn` onto `0.0.0.0`, widening an unauthenticated BQ-backed API to the LAN); `pip` pulled protobuf 7.35.1/pyarrow 24/pandas 3.0 — isolation vindicated. 8 tests, fake transport, zero live API; suite **77** green, ruff/black clean. Findings: **the API sets no timeout on its own BQ calls** (client's is the only bound in the chain); FastAPI **silently accepts undeclared query params**.
- **Warmups:** item 3 second attempt 07-21 **✅ (1/2)** — all three 07-11 misses cold (boundary, dedupe, list-arg), SCHEMA 1:1, 2/2 in-container, zero coding hints; habit gap persists: **called "Checkpoint" on a never-run file** (`.withcColumn` sat parse-green — `py_compile` is automatic, *invoking* still needs a push). Due: item 4 second pass ≥07-19 (**overdue**), test-file drill (**fresh** target, deferred from 07-21), item 1 third pass ~07-22, item 2 ~07-24 (**graded on executing per chunk**), item 3 second pass ≥07-28. Item-5 queue: pinned-backfill typing still **aided** — rep stays open.
- **Known issues:** none blocking. **Open experiment (not a finding yet):** does `PERMISSIVE` + an undeclared `_corrupt_record` mean `silver_events.py` silently *drops* unparseable lines? Queued in [decisions.md](docs/decisions.md); if confirmed, the GE counted identity is the only thing that would catch it. The API has **no timeout on its own BigQuery calls** — backlogged. `pipeline_run_metadata` holds only today's seeded row (Day-14 green/red/green history died with the destroy; rebuilds as runs happen). Docker Desktop at `AppData\Local\Programs\DockerDesktop` (non-standard). Metadata-DB persistence pattern, 3 members. Observer self-state `"running"` (accepted). Quarantine cleanup manual. `dim_date` static 2024 spine. Personal ADC — SA impersonation backlogged. DooD accepted locally. Webhook URL in `.env` (secrets backend = Phase 4). API is localhost-only until the pre-deploy gate (auth · rate limits · CORS · pinned deps).
- **Open decisions:** none — s5's calls (uncached free-path `/runs`, in-band `errors[]`, 404-on-missing-table) all logged in [docs/decisions.md](docs/decisions.md).
