...
| [docs/interview/](docs/interview/) | **Interview Q&A bank** — every question an interviewer might probe, Bryan's *verbatim* answer, the verdict, a rehearsable model answer, follow-ups, and a drill. Topic files + coverage tracker | Prepping for an interview; **after every `/quiz` — append the new questions** (format in its [README](docs/interview/README.md)) |
...
6. **Cost hygiene:** load jobs are free, query jobs bill by bytes scanned (10 MB minimum). Partition + select only needed columns. `terraform destroy` + `docker compose down` at session end. Spark/Kafka stay local in Docker. `maximum_bytes_billed` on every API query job (landed Day 15); ops reads ride free `list_rows`, never a query job.
...
| Run metadata | `devpulse_silver.pipeline_run_metadata` — unpartitioned, free `load_table_from_json` writes, free `list_rows`/`bq head` reads; `tasks`/`run_summary_json` are **`JSON`-type** (client returns them parsed); summary artifact `quality/run_summary.json` (gitignored) |
| API (Phase 3, localhost-only) | `api/` — `/trending` · `/languages/momentum` · `/leaderboard` (billed query jobs: every value a bound `@param`, `maximum_bytes_billed=100 MB`, TTL cache 300 s + `X-Cache`) · `/runs` (free `list_rows`, uncached, `errors[]` in-band) |
| Dashboard (Phase 3, localhost-only) | `dashboard/` in its own **`.venv-dashboard`** — `api_client.py` (one `_get()` → `DashboardError` carrying `status_code`; timeouts `(1,6)` marts / `(1,3)` runs) · `transforms.py` (pure; `SchemaError` guard) · `app.py` (4 panels, `st.cache_data` **300 s marts / 30 s `/runs`**). Run: `.\.venv-dashboard\Scripts\python.exe -m streamlit run dashboard/app.py` (API: `python -m uvicorn api.main:app --reload`) |
| Host test suite | **93** collected (77 after Day-16 s1 + 16 from `test_transforms.py` — 13 functions, one parametrized ×4) — Spark tests skip on host |
| Staleness (per-endpoint, not global) | marts **~600 s** worst case (Streamlit 300 + API 300, the *sum*) · `/runs` **30 s** (one layer) |
...
- [ ] **Phase 3 · Wk 6 🏁** — FastAPI over gold marts ✅ **(Day 15: 4 endpoints, parameterized + capped + cached, free-path `/runs`)**; Streamlit dashboard ✅ **(Day 16: 4 panels, honest empty/NULL/unknown states, split-TTL caching, zero SQL and zero `bigquery` import)**; **GitHub Actions CI (Day 17 — the last item)**. 🔐 Before any deploy: auth + rate limiting, pinned deps (parameterized queries ✅ Day 15 · `maximum_bytes_billed` ✅ Day 15 · CORS ✅ reframed/closed Day 16 — it is a browser policy, not a server protection). 🔐 Carried from Phase 2: pipeline-SA impersonation + silver-bucket grant.
...
- **Phase:** **Phase 3, final item in progress** — [Day 17 — hermetic CI on GitHub Actions](docs/daily/day-17.md) **step 1 of 7 done** (2026-08-26, session 1). Day 16 ✅, Day 15 ✅, Phase 2 ✅. **Phase 4 CUT** (2026-08-25 — streaming moves to FleetLoop); DevPulse now ends at **Phase 3 + Day 17 + a full README rewrite**. **Next: cut a fresh branch off `main` for step 2 — the `test` job** (expect `config.py`'s import-time `_require()` to kill *collection* until four obviously-fake env vars are supplied job-level — config, never GitHub Secrets), then steps 3–7: pin deps · `dbt parse` gate · required checks + badge · verify · docs. ⚠️ **Step 5 is now out of order:** PR #1 merged with **no branch protection and no required checks**, so the "PR blocked while red, merged when green" proof still has to happen — it will be **PR #2**. **No rehydrate, no `terraform apply`, $0 spend — still true.**
- **Day 17 s1 landed** (coach mode — Bryan wrote `ci.yml`): branch `ci/day-17` → **PR #1 merged** to `main`. `.github/workflows/ci.yml` — one `lint` job: checkout, setup-python `"3.11"`, `pip install -r requirements-dev.txt`, then `ruff check .` and `black --check --diff .` as **separate steps** (the log names which gate failed). `permissions: contents: read`, `concurrency` + `cancel-in-progress`, and **asymmetric triggers on purpose** — `push` filtered to `main`, `pull_request` unfiltered, which kills the same-repo double-run; a `branches:` filter on `pull_request` would match the **base**, not the head. **Gate proven in both directions:** green → a deliberate unused import reddened it (F401 + I001) → green again. Explicit `.` on both tools because their defaults differ (ruff defaults to `.`, black errors without a path). Commits `59f767c` · `bc0ea7f` · `a726bd0`, merged as `df27f68` — a **merge commit, not a squash**, so the two deliberate-break commits are permanently in `main`'s history (harmless, and arguably the evidence). ⚠️ **Owed:** a `#` comment on the `concurrency` block, and the **ruff/black versions CI actually resolved** (unrecorded — it is the empirical answer to whether unpinned drift has bitten, and it feeds step 3).
- **First Linux run of the host code path in 129 commits — it passed.** Portability pre-checked with evidence, not assumed: **0** files with index-side CRLF (`git ls-files --eol`), no hardcoded Windows paths in tracked Python, and `git ls-files "*.py"` = **32** = exactly the file count black reports locally, so CI's lint surface is identical to the host's. Local toolchain: ruff 0.15.20 · black 26.5.1 · pytest 9.1.1 · CPython 3.11.9. Remaining portability exposure is **step 2's** — Linux is case-sensitive and `lint` never imports anything, so a case-mismatched import has been invisible for 129 commits by construction.
- **Day 16 landed** (coach mode — Bryan wrote every panel, both cache layers, all 13 transform tests): `dashboard/` — four Streamlit panels over the API, **never** BigQuery. `Unknown` **178,081/180,386 (98.7%)** shown + captioned, never filtered; `momentum_delta` → `—` not `0`; tri-state `verdict`; `errors[]` rendered independently of `results` (the partition case had been printing "No runs available" and returning); `SchemaError` guard separating **schema drift from NULL** (13 tests, **mutation-verified 4/4**). Caching: four per-endpoint wrappers in `app.py` (not the client — `st.cache_data` is Streamlit-lifecycle memoisation, and the venv split enforces that boundary as a failing test), **split TTLs by driver** — marts 300 s (cost), `/runs` 30 s (responsiveness, free path). Commits `98ce8f4` · `3399299` · `4aedfb1`.
- **Verified:** silver/stg/fact **180,386** · dims 55,245/39,030/366/15 · marts 180,386 / 163,953 / **7,236 = fact WatchEvent count** · **PASS=69** (also the idempotency proof — grain tests would redden on an append) · three-way check all three marts · host suite **93** · ruff+black clean · `bq ls -j` **three query jobs per load, never four**.
- **Warmups:** unchanged since 07-22 — item 4 RETIRED (✅ 2/2). Due: **test-file drill** (fresh target, overdue), item 1 third pass (**overdue**), item 2 (**graded on executing per chunk**), item 3 2nd pass. Item-5 (Airflow DAG) unblocked. **New target: `dashboard/transforms.py` `_require_columns` + `SchemaError`** — written by Claude on explicit request, flagged for re-implementation.
- **Known issues:** none blocking. **Open experiment:** does `PERMISSIVE` + undeclared `_corrupt_record` mean `silver_events.py` silently *drops* unparseable lines? Queued in [decisions.md](docs/decisions.md). API has **no timeout on its own BQ calls** — backlogged. **Caches don't bound key-space exploration** — each new `limit`/`date` is 3 query jobs at both layers (mitigations named, not built). No backoff/circuit-breaker on client retries during an API outage. Docker Desktop at `AppData\Local\Programs\DockerDesktop` (non-standard). Metadata-DB persistence, 3 members. Observer self-state `"running"` (accepted). Quarantine cleanup manual. `dim_date` static 2024 spine. Personal ADC — SA impersonation backlogged. DooD accepted locally. Webhook URL in `.env` (secrets backend = Phase 4). API + dashboard localhost-only until the pre-deploy gate (auth · rate limits · pinned deps — **CORS closed/reframed Day 16**).
- **Open decisions:** `contributor_leaderboard` rank 1 is **`github-actions[bot]` (19,082 = 11.6%)** — label it, filter it, or leave it? Silent filtering would be the `Unknown` lie again. **Undecided, nothing filtered.** Enrichment-at-scale design (GraphQL batch + incremental anti-join behind the 4-row seed) banked in [decisions.md](docs/decisions.md).
