...
- **Phase:** **Phase 3 in progress** — [Day 15](docs/daily/day-15.md) **complete ✅** (all 8 steps; s1–s5, 2026-07-12→17). Phase 2 complete ✅ (Days 8–14). **Next session:** `/plan-day` → **Day 16: dashboard over the API** (then GitHub Actions CI).
- **Day 15 s5 landed** (07-17): `/runs` over **free `client.list_rows`** — **zero query jobs, proven against BQ job history**; read-all → Python sort desc → slice; **deliberately uncached** (free read saves nothing · ops window must not hide the run you just kicked · run metadata is the mart cache's invalidation key — caching it is circular). NULL rows served honestly (`verdict: null`); malformed rows → structured `errors[]` in-band; missing table = routine post-destroy state → deliberate 404. Table seeded via pinned backfill (`--reset-dagruns`, 7/7 green, 117 s). Step 6: Phase-0 slice retired (−106 lines). Findings banked: BQ `JSON` columns arrive **parsed** ("artifact wins" #3 — `bq show --schema` beat reviewer memory); two **born-vacuous tests** (pre-sorted fixture; no-op limit) → mutation discipline (delete the code under test, demand red). Suite **69** green (30 API), ruff/black clean; full reconciliation held (180,386 silver/stg/fact · 163,953 · 7,236 = WatchEvents · **PASS=69** on second MERGE build).
- **Warmups:** item 2 third attempt 07-17 **🔶** — content ≈ 1:1 cold (both 07-10 prompted items landed unprompted); habit: `py_compile`-per-function now automatic, but **execution still only when pushed** (5 runtime-class bugs sat parse-green). Due: item 3 ~07-18, item 4 second pass ≥07-19, test-file drill ~07-21 (**fresh** target), item 1 third pass ~07-22, item 2 ~07-24 (**graded on executing per chunk**). Item-5 queue: pinned-backfill typing was **aided** today (inclusive-range semantics missed twice) — rep stays open.
- **Known issues:** none blocking. `pipeline_run_metadata` holds only today's seeded row (Day-14 green/red/green history died with the destroy; rebuilds as runs happen). Docker Desktop at `AppData\Local\Programs\DockerDesktop` (non-standard). Metadata-DB persistence pattern, 3 members. Observer self-state `"running"` (accepted). Quarantine cleanup manual. `dim_date` static 2024 spine. Personal ADC — SA impersonation backlogged. DooD accepted locally. Webhook URL in `.env` (secrets backend = Phase 4). API is localhost-only until the pre-deploy gate (auth · rate limits · CORS · pinned deps).
