...
- **Phase:** **Phase 3 in progress** — [Day 15](docs/daily/day-15.md) (FastAPI over the gold marts), **steps 0–3 of 8 done** (s1 2026-07-12→13, s2 07-13, s3 07-14). Phase 2 complete ✅ (Days 8–14).
- **Day 15 s3 landed** (`ae67761` endpoints + `bf5b0eb` tests): `/languages/momentum` + `/leaderboard` live; `momentum_delta` typed `int | None`, **no default** (NULLABLE proven via `bq show` — mode *omitted* = nullable; NULL on 100% of one-hour rows, test-pinned); `/leaderboard` offset bound raised (32,681 rows/day made the copied `le=10_000` tail-unreachable — deep-offset test pins the raise). **Endpoint-vs-warehouse reconciliation all ✅: 7,236 / 180,386 / 163,953.** Suite **50** green; ruff/black clean. **Bryan wrote all 23 endpoint tests — zero handouts** (the morning test-file drill transferred same-day).
- **Next session:** `/start-session` full rehydrate (destroy ran), then Day 15 step 4 (TTL cache — monotonic clock, fake-clock pytest, `X-Cache` header) + step 5 (`/runs` via free `list_rows`), then 6–8 (retire Phase-0 slice · verify · docs incl. skills-map + tradeoffs).
- **Warmups:** test-file drill run 07-14 **🔶** (fixture-resolution error + 5 pointer-found defect classes; structure 1:1 cold). Due: item 1 repeat **~07-15**, item 2 ~07-17, item 3 ~07-18, item 4 second cold pass ≥07-19; test-file repeat ~07-21 on a **fresh** target (`test_load_pipeline_metadata.py` or the `test_queries.py` DI test), graded on per-test red→green.
...
- **Open decisions:** none — `?date=` **required** confirmed deliberate (s3; a literal default = plausible stale 200s) and the offset-bound shape resolved (bounds encode cardinality assumptions); both in [docs/decisions.md](docs/decisions.md).
