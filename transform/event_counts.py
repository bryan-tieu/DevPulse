"""DEPRECATED (Day 7) — the Phase-0 thin-slice in-memory Counter.

Superseded in the pipeline by the PySpark silver job + transform/load_silver.py:
the silver_events table now carries cleaned, typed, deduped events, so this no
longer runs in the Airflow DAG. Kept (not deleted) only because api/main.py still
reads its `hourly_event_counts` table and run.py still calls it — both retire in
the Phase 3 API rework, when the API moves onto the silver/gold models.
"""
