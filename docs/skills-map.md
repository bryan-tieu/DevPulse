...
| Orchestration (Airflow) | ✅ | `airflow/dags/devpulse_ingest.py`: sensor → ingest → Spark → load → **dbt gate**; retries, reschedule-mode sensor, bounded 48h backfill, pinned-interval triggers (and the off-by-one war story) | "My tasks are pure functions of the data interval — that's what makes them retryable and backfillable." |
...
| Data quality gates | 🔶 | **DAG-level gate proven (Day 12):** `dbt_build` DockerOperator fails the run on any red test — first live catch was real bad data (out-of-spine hour → 4 relationships failures). GE bronze→silver = Day 13 | "Quality checks are gates that fail the run, not dashboards nobody reads — mine caught real bad data on its first live run." |
...
| Observability / run metadata & alerting | ⬜ | Phase 2 back half — failure alerting + run-metadata table | — |
