...
| Orchestration (Airflow) | ✅ | `airflow/dags/devpulse_ingest.py`: sensor → ingest → Spark → load; retries, reschedule-mode sensor, bounded 48h backfill | "My tasks are pure functions of the data interval — that's what makes them retryable and backfillable." |
...
| Data quality gates | 🔶→⬜ | dbt tests gate gold today; Great Expectations bronze→silver + DAG-level gate = Day 12+ (Phase 2 back half) | "Quality checks are gates that fail the run, not dashboards nobody reads." |
