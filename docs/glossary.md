...
**Error translation at a boundary** — Converting foreign exceptions into your own vocabulary at the edge of a layer, so callers never see the transport library's types. DevPulse: `api/main.py` turns `google.api_core` errors into `HTTPException`; `dashboard/api_client.py` turns `requests` exceptions into a single `DashboardError`. The seam is what lets the underlying library be swapped without touching consumers — and the day a `requests`-shaped error reaches a render function, the UI has silently coupled itself to the transport.

**Connect vs read timeout** — The two clocks in an HTTP request: **connect** bounds the TCP handshake ("is anything listening?"), **read** bounds waiting for the response body ("is it still working?"). `requests` accepts them as a tuple, `timeout=(connect, read)`; it has **no default**, so omitting it means an unbounded hang with no error to alert on. A timeout is a *correctness* property, not tuning — it converts a hang into a bounded, reportable failure. DevPulse: `(1, 6)` for mart endpoints (a cold-cache BigQuery query job measured 0.9–1.2 s), `(1, 3)` for `/runs` (free `list_rows`, measured flat at ~0.2 s — slow there means *wrong*, not cold).

**Fake / stub / mock / spy** — Test doubles, by what they're for. A **stub** returns canned answers; a **fake** is a working lightweight implementation; a **mock** carries expectations about how it's called; a **spy** records calls so you can inspect them afterward. DevPulse: `FakeResponse` is a fake (only the surface `_get` touches — `.ok`, `.json()`, `.status_code`); the timeout tests are spies (they assert on `call_args.kwargs`, not on a return value); Day 15's `dependency_overrides` fakes injected a fake BQ client.

**Vacuous (born-dead) test** — A test that cannot fail: `assert error.detail in error.detail`, a fixture that already satisfies the property under test, a limit that slices nothing. It passes forever and asserts nothing, which is worse than no test because it reports coverage it doesn't provide. The only proof a test is real is the **mutation check**: break the code it claims to guard and demand red. Three occurrences banked in DevPulse (two on Day 15 s5, one on Day 16 s1) — every one discovered green.
...
**CORS (cross-origin resource sharing)** — A **browser** policy, enforced against **JavaScript**, deciding which web origins' scripts may *read* a response. It is not a server protection: it authenticates nobody and stops no non-browser client (`curl`, `requests`, a server-rendered framework). Adding permissive CORS **reduces** a browser-side restriction rather than adding a server-side control. It never fired in DevPulse because Streamlit renders server-side — `requests` runs in the Streamlit process, the browser only talks to `:8501`, and a server-side client sends no `Origin` header, so there is no decision to make. Becomes required the day a JS frontend calls the API from another origin; never a substitute for auth or rate limiting.

**Cache staleness stacking** — When caches sit **in series**, worst-case staleness is the **sum** of their TTLs, not the larger of them: the far cache can already hold a value 299 s old when the near cache fetches it, then holds it another 300 s. DevPulse: the marts stack `st.cache_data(ttl=300)` on `api/cache.py`'s 300 s → **~600 s**; `/runs` has one layer only (uncached server-side by Day-15 design) → **30 s**. The staleness figure is therefore **per-endpoint**, never one number for a system. Corollary: a refresh button clears only the layer it owns, so it can promise freshness it cannot deliver — which is why DevPulse's is labelled "Clear Cache Data", not "Refresh".

**Invalidation key (vs. TTL)** — A token derived from the producer that changes exactly when the data changes, used as part of the cache key so a new version is automatically a miss at every layer. For batch pipelines the honest key is the newest `run_id` in the run-metadata table. TTL is the approximation you accept when it isn't wired: uncorrelated with the event, so it refetches identical data between runs *and* serves stale data right when new data lands. Same pattern as content-hashed asset filenames — you never expire the cache, you change the key. Cannot be applied to the endpoint that *returns* the key (circular).

**Tri-state rendering** — Displaying a nullable field as **three** distinct states rather than coercing to two. `verdict: bool | None` is pass / fail / **unknown**; `momentum_delta: int | None` is a number or `—`, never `0` (`0` asserts "no change measured," a claim the data doesn't support). Collapsing `null` into either pole is a lie an operator would act on — the last-mile version of an INNER join dropping rows.

**Schema drift vs. null** — Two failures that a `.get()` default silently merges. A **null value** is a real data state the pipeline preserved deliberately; an **absent key** means the producer's contract moved, or the consumer names a field that doesn't exist. Only the second is a bug, and it must fail loudly. DevPulse: `_require_columns()` raises `SchemaError` (distinct from `DashboardError`, which means the *call* failed rather than the *body* being wrong) and reports every missing column at once, where a bare `KeyError` stops at the first and speaks Python's vocabulary instead of the domain's.

**Cache key space** — The set of distinct argument combinations a cache can hold. A cache prevents re-fetching **the same key**; it does nothing about a user exploring *new* keys. DevPulse: a 1–100 slider crossed with N dates is up to 100·N keys per endpoint, three query jobs each at the 10 MB floor — visible in `bq ls -j` as clusters of three, seconds apart. Mitigations: coarser controls, debounce, or over-fetch once and slice client-side (why real dashboards paginate in the browser).
