...
- **Current day:** [Day 9 — Deepen the star schema: dim_repo / dim_actor / dim_date](daily/day-09.md) ✅ complete (3 dims added — surrogate keys + Type-1 + generated calendar; `dbt build` PASS=28)
...
- **Next up:** Day 10 — the **incremental `fact_events`** (its own meaty materialization lesson: `is_incremental()`, a unique key, late-arriving rows) joining the four dims via their keys, plus the first gold **mart** and the **`relationships`** tests (fact FK → dim PK) that the dims were built to satisfy. Still pending from Phase 1: pipeline-SA auth (impersonation) + the silver-bucket SA grant; expanding the bounded 48h backfill to a full week on demand.
