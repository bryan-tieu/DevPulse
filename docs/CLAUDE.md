...
- **Current day:** [Day 7 — silver → BigQuery + wire Spark into Airflow](daily/day-07.md) ✅ complete (**Phase 1 PySpark milestone done**)
...
- **Next up:** Day 8 — **Phase 2 begins: dbt** (staging → `dim_`/`fact_` star schema → marts) over `silver_events` in BigQuery; incremental fact, tests + docs. Still pending from Phase 1: pipeline-SA auth (impersonation) + the silver-bucket SA grant; expanding the bounded 48h backfill to a full week on demand.
