...
- **Current day:** [Day 8 — Phase 2 begins: dbt bootstrap + first vertical sliver](daily/day-08.md) ✅ complete (dbt scaffolded, `source → stg_events → dim_event_type` green)
...
- **Next up:** Day 9 — **deepen the star schema**: `dim_repo`/`dim_actor` (surrogate keys via `dbt_utils.generate_surrogate_key`), `dim_date`, then the **incremental `fact_events`** (its own meaty lesson) and the gold marts (`trending_repos_daily`, etc.). Still pending from Phase 1: pipeline-SA auth (impersonation) + the silver-bucket SA grant; expanding the bounded 48h backfill to a full week on demand.
